...
- **BLE is now offered on the ESP32-P4 in the Config Wizard.** The P4 was marked `supports: { ble: false }` on the reasoning that the chip has no radio — true (there is no `SOC_BT_SUPPORTED` in `soc/esp32p4/include/soc/soc_caps.h`, and IDF's `BT_CONTROLLER_ENABLED` depends on it), but not disqualifying. ESPHome's `esp32_ble` detects an `esp32_hosted` config and switches to `CONFIG_BT_CONTROLLER_DISABLED` + `CONFIG_ESP_HOSTED_ENABLE_BT_BLUEDROID` + `CONFIG_ESP_HOSTED_BLUEDROID_HCI_VHCI`, running the Bluedroid *host* on the P4 against the ESP32-C6 companion's controller over VHCI; `ble.cpp`, `esp32_ble_tracker`, `esp32_ble_beacon` and `bluetooth_proxy` all branch on that symbol. So `cca_source: ble` works on the P4 over the same SDIO link that carries Wi-Fi. No new partition table is needed: unlike the 8 MB boards, where BLE forced the app slots from 1.75 MB to 2.0 MB, the P4's existing `tigo-16mb.csv` 3 MB slots fit the BLE build at 1.80 MB (49%), so enabling BLE on a P4 requires no repartition. Adds `boards/test-p4-ble-tigomonitor.yaml` as a local-components compile check; it builds clean on IDF 5.5.5 with all six hosted-BT sdkconfig symbols resolved on an `esp32p4` target. Confirmed on an EV board: the firmware boots, Wi-Fi over the C6 is unaffected (RSSI -40, `/api/status` healthy), `USE_TIGO_CCA_BLE` is live, and a CCA refresh drives `esp32_ble_client` to `Connecting` — which means `esp_hosted_connect_to_slave()`, `esp_hosted_bt_controller_init()`/`_enable()`, Bluedroid host init and GATT-client registration all succeeded on a chip with no radio. A full CCA session then succeeded once the board was in range: DEVICE_INFO + NETWORK_INFO pulled over BLE (607 + 284 bytes of real payload) and persisted to NVS.
  ...
  **Known limitation — one BLE session per boot.** After that first session, every subsequent connect is refused instantly (sub-millisecond, no radio activity) with `BT_HCI: opcode=0x2043, status= 0c` — LE Extended Create Connection / Command Disallowed — followed by `ESP_GATTC_OPEN_EVT in DISCONNECTING state (status=133)`. The C6 controller still believes an ACL link exists; the CCA does not (another central connects to it fine), and ESPHome's host-side client self-heals to IDLE on the failed open (`ble_client_base.cpp:306`), so the stale state sits past the VHCI boundary on the C6. Confirmed by reboot-and-retest: refresh #1 after a reboot changes the CCA's `uts` nonce, refresh #2 does not. Whether this is specific to the hosted transport or a teardown bug that a native controller merely tolerates is not yet established — a valid same-code control on an S3 with BLE scanning enabled is still needed. Ruled out so far: **C6 slave firmware version** (updating the co-processor to the latest published `esp-hosted-firmware` image reproduced the identical 1-of-3 pattern), and **host/slave version skew** (ESPHome's `ref=` is a minimum, not a pin — the build resolves `esp_hosted` 2.12.9 against a 2.12.x slave, so the two halves were never far apart). Until then, prefer `cca_source: http` on the P4.
