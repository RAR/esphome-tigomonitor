...
### Fixed
- **Opening the Diagnostics page could reboot the device.** The page asked for `/api/tsdb/stats` every 10 seconds, and building that response is the only thing in the whole web UI that makes the device touch flash. On the ESP32-S3 every flash access stalls the other core, and a stream of them from the web server interleaved with the history writer's own faults the writer mid-write. Reproduced on demand: four concurrent callers killed the device in about eleven seconds, while four concurrent callers of `/api/status` — same load, no flash — ran indefinitely. The response is now built at most once a minute and served from PSRAM in between, and the page asks for it on open rather than on every tick. The numbers in it only change when the writer commits, so nothing is lost. This does not fix the underlying flash-versus-cache fault, which can still reboot the device roughly once a day; it removes the way the UI was provoking it.
