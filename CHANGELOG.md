...
### Changed
- **The `esp_tsdb` fork is retired; the board configs pin `zakery292/esp_tsdb^2.4.1` from the registry.** Both changes the fork existed for shipped upstream in 2.4.0 — the sidecar header write ([PR #6](https://github.com/zakery292/esp_tsdb/pull/6)), without which a history snapshot costs 21.4 s instead of 633 ms, and the `esp32p4` manifest target ([PR #4](https://github.com/zakery292/esp_tsdb/pull/4)), without which the component manager refuses to install on a P4 at all. Six board configs, the Config Builder and the docs drop `source:`/`ref:` for a plain version spec. The third commit the fork carried, `tsdb_peek_span`, turned out never to have been called by this firmware, so nothing was lost by dropping it. See [`docs/esp-tsdb-fork.md`](docs/esp-tsdb-fork.md).
  - `^2.4.1` is a floor, not a preference. Anything below 2.4.0 rewrites the database header in place at offset 0 on every commit, which LittleFS turns into a rewrite of every block to EOF — and it does not fail loudly, it just spends ~20 s of every snapshot interval inside a flash write. Do not relax it.
...
### Fixed
- **Builds fail on ESPHome 2026.9.0 with `cJSON.h: No such file or directory`.** 2026.9.0 stopped compiling most of ESP-IDF's built-in components unless something in the build asks for them, and two of ours were never asked for: `json` (cJSON, on IDF 5.x) and `esp_http_server`. The first broke every config; the second was hidden on most because `captive_portal` happens to request it, and surfaced on configs without one. Both are requested now. Nothing to change in your YAML — rebuild against `main` (or set `refresh: 0s` if your `external_components:` pins `ref: main` and picked up a stale clone). Reported in [#71](https://github.com/RAR/esphome-tigomonitor/issues/71); the `esp_http_server` half is @davidcoulson's [#70](https://github.com/RAR/esphome-tigomonitor/pull/70). Earlier ESPHome versions never excluded these, so the change is a no-op there.
- **A wrong UART baud rate is now a config error, not a log line after boot.** `tigo_monitor` used to check for 38400 8N1 at `dump_config()` time and log an error if the bus was set to anything else; ESPHome 2026.9.0 deprecates that check (it also printed a compiler warning on every build), so the same requirement is now enforced when the config is validated, along with a required `rx_pin`. A config that already builds is unaffected.
- **The Config Builder emitted an invalid `manual_ip:` block.** Filling in the optional "Static IP" field produced `manual_ip:` with `static_ip:` and nothing else, but ESPHome marks `gateway` and `subnet` **required** — so every generated static-IP config failed validation before it ever reached a compiler. The builder now emits all five fields, deriving `gateway`/`dns1` from the address on the assumption of a `/24` with the router at `.1`, and saying so in a comment.
  - `dns1`/`dns2` are the ones that matter beyond validation: they're optional and default to `0.0.0.0`, so a hand-written block that supplies gateway and subnet but omits DNS builds and runs, and simply cannot resolve a hostname. SNTP never answers, the clock stays at epoch 0, history records nothing, and the dashboard keeps working over the static address the whole time. That was the second half of the root cause in [#60](https://github.com/RAR/esphome-tigomonitor/issues/60), and it's now documented as its own troubleshooting entry.
- **Generated configs set `refresh: 0s` on `external_components:`.** ESPHome caches a git source for a day by default, so a `ref: main` build could serve a clone up to 24 hours stale — including one missing a fix a user had just been told to pick up, with nothing in the build output to say so ([#60](https://github.com/RAR/esphome-tigomonitor/issues/60)).
- **Docs: the first-run failures behind [#60](https://github.com/RAR/esphome-tigomonitor/issues/60) are now written down.** A new Network section in the troubleshooting guide covers the DNS-less static IP and the stale component cache; the TSDB section gains "History Empty on a Device That Is Otherwise Working", which separates an unset clock from a storage failure and gives the `0 records / 0 writes` test for telling them apart. The history guide notes that linking `time_id` is only half the job — the time source has to be reachable — and offers `platform: homeassistant` as the way around NTP entirely. Getting-started points at the clock before anything else. The troubleshooting guide also stopped telling people to pin the retired `esp_tsdb` fork by commit SHA, and the BLE section now says to pick the CCA MAC from the scan rather than type it.
- **History tab reported "on-flash history failed to start" when the device clock was unset.** `/api/history/power` and `/api/history/panel` return 503 for two unrelated reasons — the database failed to open, or there is no valid wall-clock to build a time range from. Only the first carried a `detail`, so an unsynced SNTP server rendered as a storage failure and sent a reporter auditing their partition table while the database was open and healthy (#60).
- **A History tab that reports `HTTP 503` now says which failure it is.** The four `/api/history/*` routes answered 503 with `history not initialized` — true, and useless: the three ways `init()` can fail (no `tsdb` partition in the running flash layout, a partition that will not mount, a database that will not open) have entirely different fixes. The reason is now recorded at init and returned as `detail`, and the History view shows it instead of the status code ([#60](https://github.com/RAR/esphome-tigomonitor/issues/60)). The common case names itself: the firmware was built with a `partitions:` line, but the table was never sent — `esphome run` does not send one and reports success anyway. Together with the 404 message added earlier, the two codes are now self-explaining: 404 means history was never compiled in, 503 means it was and could not start.
- **A node table naming an unknown MPPT no longer produces an empty dashboard.** In the node JSON, `cca_mppt` holds the **MPPT** label — the inverter itself comes from the YAML `inverters:` block, and a string is attached to an inverter by matching that label against its `mppts:` list. A hand-written table that reads the old field name (`cca_inverter`) literally and puts the inverter's name there imported successfully, built every string, and returned them all from `/api/strings` — but `/api/inverters` silently dropped any string it could not attach, so the Dashboard and Topology views, which read that endpoint, showed **0 strings / 0 panels** while 25 panels reported live telemetry ([#60](https://github.com/RAR/esphome-tigomonitor/issues/60), reported by @kidrock1970). Import now reports unmatched labels back, `setup()` logs the same check against the stored table on boot, and any string that matches no inverter is grouped under **Unassigned** instead of vanishing — which also covers having no `inverters:` block at all, where every string was unclaimed and the dashboard was simply blank.
  - The field is renamed `cca_mppt`, since the old name is what invited the mistake. It comes from the CCA, which calls an MPPT an "Inverter". `cca_inverter` is still accepted on import and still emitted on export, so existing files and older firmware keep working.
  - The import format had never been documented beyond one line saying it "replaces the node table". [Web Server](https://rar.github.io/esphome-tigomonitor/guides/web-server/) gains a Node table format section with the field table and the inverter → MPPT → string model written out.
- **The AtomS3R config had the wrong UART pins, and so did the config builder.** `boards/esp32s3-atoms3r.yaml` set `tx_pin: GPIO1` / `rx_pin: GPIO2` and described them as a "built-in tail485 RS485 transceiver". The AtomS3R has no on-board transceiver, and G1/G2 are its Grove port — the M5Stack Atomic RS485 Base, which the wiring guide names as the recommended pairing and which the development rig actually runs, sits on the bottom header at **GPIO5 (RX) / GPIO6 (TX)**. Anyone who copied the shipped board file or generated a config from the builder got firmware that booted, joined Wi-Fi, served the web UI, and never received a single frame, with nothing in the log to explain it ([#60](https://github.com/RAR/esphome-tigomonitor/issues/60), reported by @kidrock1970). The ESP32-P4 board file had the same defect from the other direction — `GPIO1`/`GPIO3` placeholders, where every P4 config that has actually been run uses `GPIO20`/`GPIO21`.
  - The drift guard now checks `tx_pin`/`rx_pin` between each board file and the builder's board data, which is what caught the P4 case. A wrong pin is the worst kind of config bug: it cannot fail loudly, because there is nothing to fail.
- **`cca_source: ble` against a partition table too small for the BLE stack is now a config error.** Folding `ble_client`/`esp32_ble` into `tigo_server` pushes the image past the stock 1.75 MB OTA slot, which is why `boards/partitions/tigo-8mb-ble.csv` exists with 2.25 MB slots. Pairing BLE with the non-BLE table was accepted, compiled the entire firmware, and then failed as a linker overflow or an OTA the device rejected for not fitting — neither of which mentions partitions, and the fix is a file nowhere near the error. Validation now names the slot sizes and the table to use, and warns that the BLE table moves `nvs` (export the node table first). Only checked when `partitions:` names a readable CSV; with no explicit table the slot size is unknowable and nothing is rejected.
  - The History tab no longer reports a bare `HTTP 404` on a build without on-flash history. The `/api/history/*` routes are registered only when `TIGO_TSDB_AVAILABLE` is defined — that is, when the YAML pulls in `esp_tsdb` and `littlefs` and gives them a partition — so on a build without them the endpoint genuinely does not exist. A status code is a true statement of what happened and a useless one about why; it sent people looking at their CCA and their wiring. It now names the missing dependency.
  - Troubleshooting gains a section on telling "no bytes are arriving" apart from "bytes are arriving but never frame" using the DEBUG `Buffer:` line and the Missed Frame Count sensor. They present identically as zero devices and have unrelated causes.
- **The config builder now wires up a time source, and a missing one is no longer silent.** Nothing the wizard generated ever set `time_id:`, and four of the five ready-to-flash examples either omitted the `time:` block or defined one and never referenced it. A `time:` block on its own does nothing: without `time_id:` the component's clock pointer stays null, and the daily energy totals, the midnight reset and every on-flash history snapshot quietly do nothing — the snapshot skip logged only at DEBUG, below the INFO level those same configs set ([#58](https://github.com/RAR/esphome-tigomonitor/issues/58), reported by @crocri). Generated configs now emit an SNTP `time:` block and the matching `time_id:`, the shipped examples link the time sources they already declared, and `dump_config` says so at boot when `time_id` is absent.
...
[Unreleased]: https://github.com/RAR/esphome-tigomonitor/compare/v2.0.1...HEAD
[2.0.1]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.1
...
[2.0.0-beta.4]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.0-beta.4
[2.0.0-beta.3]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.0-beta.3
[2.0.0-beta.2]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.0-beta.2
[2.0.0-beta.1]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.0-beta.1
[2.0.0-alpha.1]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v2.0.0-alpha.1
[1.4.5]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.5
[1.4.4]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.4
[1.4.3]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.3
[1.4.2]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.2
[1.4.1]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.1
[1.4.0]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.4.0
[1.3.1]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.3.1
[1.3.0]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.3.0
[1.2.0]: https://github.com/RAR/esphome-tigomonitor/releases/tag/v1.2.0
