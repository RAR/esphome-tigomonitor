...
- **Internal temperature showed a misleading 0 °C instead of N/A** (#28). `internal_temp` initialized to `0.0f` and the return of `temperature_sensor_get_celsius()` was never checked — so when the die-temp peripheral is absent (the classic ESP32 lacks it; install fails and the handle stays null) or a read errors, the dashboard rendered `0 °C` as if real. The API now emits JSON `null` when no reading is available and the UI shows **N/A**; the measurement range was also widened (−10…110 °C) so a hot S3 die doesn't fall out of range and error.
