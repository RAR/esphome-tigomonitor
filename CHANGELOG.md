...
### Added
- **BLE is now offered on the ESP32-P4 in the Config Wizard.** The P4 was marked `supports: { ble: false }` on the reasoning that the chip has no radio — true (there is no `SOC_BT_SUPPORTED` in `soc/esp32p4/include/soc/soc_caps.h`, and IDF's `BT_CONTROLLER_ENABLED` depends on it), but not disqualifying. ESPHome's `esp32_ble` detects an `esp32_hosted` config and switches to `CONFIG_BT_CONTROLLER_DISABLED` + `CONFIG_ESP_HOSTED_ENABLE_BT_BLUEDROID` + `CONFIG_ESP_HOSTED_BLUEDROID_HCI_VHCI`, running the Bluedroid *host* on the P4 against the ESP32-C6 companion's controller over VHCI; `ble.cpp`, `esp32_ble_tracker`, `esp32_ble_beacon` and `bluetooth_proxy` all branch on that symbol. So `cca_source: ble` works on the P4 over the same SDIO link that carries Wi-Fi. No new partition table is needed: unlike the 8 MB boards, where BLE forced the app slots from 1.75 MB to 2.0 MB, the P4's existing `tigo-16mb.csv` 3 MB slots fit the BLE build at 1.80 MB (49%), so enabling BLE on a P4 requires no repartition. Adds `boards/test-p4-ble-tigomonitor.yaml` as a local-components compile check; it builds clean on IDF 5.5.5 with all six hosted-BT sdkconfig symbols resolved on an `esp32p4` target. Confirmed on an EV board: the firmware boots, Wi-Fi over the C6 is unaffected (RSSI -40, `/api/status` healthy), `USE_TIGO_CCA_BLE` is live, and a CCA refresh drives `esp32_ble_client` to `Connecting` — which means `esp_hosted_connect_to_slave()`, `esp_hosted_bt_controller_init()`/`_enable()`, Bluedroid host init and GATT-client registration all succeeded on a chip with no radio. A full CCA session then succeeded once the board was in range: DEVICE_INFO + NETWORK_INFO pulled over BLE (607 + 284 bytes of real payload) and persisted to NVS.

  **Repeated BLE sessions require `CONFIG_BT_BLE_50_FEATURES_SUPPORTED: "n"`, which selecting BLE on a P4 now sets automatically.** Without it only the *first* CCA connect after a boot succeeded; every later one was refused sub-millisecond with `BT_HCI: opcode=0x2043, status= 0c` (LE Extended Create Connection / Command Disallowed), surfacing as `ESP_GATTC_OPEN_EVT in DISCONNECTING state (status=133)`. Root cause, from a Bluedroid HCI trace at `CONFIG_BT_LOG_HCI_TRACE_LEVEL_DEBUG`: the two attempts issue *different commands*. The working one enqueues `opcode=0x200d` — `LE_Create_Connection`, the legacy initiator — and completes via an `LE Meta 0x0A` Enhanced Connection Complete. Once a session has completed, Bluedroid switches to `btsnd_hcic_ble_create_ext_conn` / `opcode=0x2043`, and the C6 refuses that command. The switch is triggered by a *completed session*, not by boot: in one run connect #1 timed out without establishing a link, so #2 still used the legacy path and succeeded, and only #3 went extended and was refused. The selector is `p_lcb->is_aux` in `l2c_ble.c`, and compiling BLE 5.0 out removes the extended branch from `l2cble_create_conn` entirely, keeping the legacy path the C6 honours. Verified on an EV board: 3/3 consecutive refreshes advance the CCA's `uts` nonce, 3× `0x200D`, zero `0x2043`, zero `Cmd Disallowed`. The trade is extended advertising/scanning, which the CCA does not use — relevant only if you also want a `bluetooth_proxy` on the same P4.

  Two earlier hypotheses were **wrong** and are recorded so they aren't retried. It is *not* a teardown-timing problem: the HCI trace shows the disconnect is sent (`opcode=0x406`, `rsn:0x13`) and fully acknowledged by the C6 (`Command Status`, then `Disconnection Complete`, `hcif disc complete`) within 115 ms, so there is no dropped close and no link the controller failed to release. Widening the post-command settle window from 500 ms to 3000 ms was tried and reverted; it changed nothing. Also ruled out: **C6 slave firmware version** (updating the co-processor reproduced the identical pattern) and **host/slave version skew** (ESPHome's `ref=` is a minimum, not a pin — the build resolves `esp_hosted` 2.12.9 against a 2.12.x slave).
...
### Fixed
- **ESP32 die temperature always reported `null`.** `/api/status` has been returning `"internal_temp": null` because the sensor never installed. `temperature_sensor_install()` requires the requested range to fit inside a *single* hardware range — its `choose_best_range()` scans for one table entry satisfying `range_min <= requested_min && range_max >= requested_max` and fails the install with `ESP_ERR_INVALID_ARG` ("Out of testing range") if none does. The ESP32-S3 table is `{50..125, 20..100, -10..80, -30..50, -40..20}`, and we asked for **-10..110**, which spans two entries and matches none. The comment claimed the wide range was there so a hot die "still reads in-range rather than erroring out and reporting nothing" — it caused exactly that failure instead. Now requests `-10..80`: a real entry, the most accurate one (±1 °C), and the same range ESPHome's own `internal_temperature` platform uses. A hot die is not a concern — the configured range is only a starting hint, since `temperature_sensor_get_celsius()` reports `range_changed` and follows the hardware onto another range at read time, hard-failing only outside the sensor's absolute -40..125 span.
- **A TSDB database whose stats call timed out vanished from the History diagnostics table.** `tsdb_get_stats_h()` acquires the per-database mutex with a 5 s timeout, and the writer task holds that mutex across `tsdb_sync_h()`'s `fclose`/`fopen` — so a stats poll landing on a flush logs `tsdb_get_stats_h: lock timeout` and returns `ESP_ERR_TIMEOUT`. `/api/tsdb/stats` treated that identically to "database not open" and emitted no row, so the table silently lost a database with nothing to indicate why. It now emits the row with `available:false`, the `esp_err` name, and `null` for every numeric field; the page renders "—" and explains itself in the range column. Also fixes a latent JSON bug on the same path: the caller cleared its `first` flag even when the row had been skipped, so a skipped *first* database produced a leading comma and invalid JSON.
...
### Changed
- **Config Builder and docs synced with the new sdkconfig defaults.** The wizard's board data now emits `CONFIG_MBEDTLS_EXTERNAL_MEM_ALLOC` (AtomS3R) and `CONFIG_VFS_SUPPORT_DIR` (AtomS3R + P4), matching `boards/*.yaml`; `boards/esp32p4-evboard.yaml` gained `CONFIG_VFS_SUPPORT_DIR` too, since the P4 runs the same History/tsdb stack and had the same dead size column. New troubleshooting entries cover the 0 KB size column, the `stats unavailable` row, and a die temperature that reads nothing. The TSDB guide documents both `/api/tsdb/stats` caveats.
- **Corrected a stale root-cause note in the TSDB guide.** The reboot-wipes-history bug (upstream `zakery292/esp_tsdb#1`) was attributed to `esp_littlefs` returning `ENOENT` from `stat()` for files that `fopen` could read. That was almost certainly `CONFIG_VFS_SUPPORT_DIR` being off — which compiles the VFS directory syscalls out entirely, so `stat()` fails for every path while `fopen`/`fread`/`fwrite` keep working. The fopen-based existence probe remains the right fix; the attribution was wrong.
- **mbedtls TLS buffers moved to PSRAM** (`CONFIG_MBEDTLS_EXTERNAL_MEM_ALLOC`). The cloud (`cloud_import`) TLS path re-parses the pinned CA on every request (keep-alive is off), and mbedtls defaulted its SSL in/out content buffers plus handshake/X.509 state to **internal** heap — dragging the internal-RAM floor down hard during each handshake. Routing mbedtls allocations to PSRAM (dependency satisfied via `SPIRAM_USE_CAPS_ALLOC`) raised the measured idle `min_free_heap` on the AtomS3R reference build from ~50.8 KB to ~62.6 KB (**+~11.8 KB**). Orthogonal to `require_mbedtls_sha512()`, which only re-enables the SHA-384/512 algorithms — not the allocator mode — so the two compose without conflict. Added to `boards/esp32s3-atoms3r.yaml`. Confirmed under load as well as at idle: three live cloud TLS round-trips (health + equipment latest/history) moved `min_free_heap` by **zero bytes**, where a single handshake previously dragged the floor to ~18.6 KB.
- **Cloud API response bodies now accumulate in PSRAM.** `cloud_http_json_()` already read into a PSRAM chunk buffer but appended into a default-allocator `std::string`, so every Tigo cloud response — system layout, equipment status/history, login — landed on the internal heap. The `MAX_RESP` cap is 96 KB, larger than the entire internal heap, so a big layout response on a large system could crater the floor or OOM outright before ever reaching the cap. The accumulator and all five call-site buffers are now `psram_string`, and `tigo_cloud_health()` / `tigo_cloud_equipment()` hand their result to the web layer by move rather than copying it into a second internal buffer.
- **All cJSON parsing now allocates from PSRAM, process-wide.** The PSRAM allocator hooks were previously installed and torn down around three specific parse sites, which left every *other* `cJSON_Parse` on the default internal allocator — most importantly `/api/nodes/import`, which accepts up to 100 KB of JSON and expands it to roughly 2–4× that in cJSON nodes. Because cJSON's hooks are global state, the set/reset pattern was also a latent race between the httpd task and the main-loop CCA sync. The hooks are now installed once from `setup()` and never reset (`tigo_cjson_use_psram()`), which closes both. Mixing allocators across the install boundary is safe on ESP-IDF — `heap_caps_free` accepts internal pointers and `free` accepts PSRAM ones. The import handler's node vector became a `psram_vector` to match. Verified on-device: a 40-node import round-trip, a cloud layout import, and the cloud health/equipment proxies all return correct data on the new path.
- **CCA local-HTTP responses and cloud credentials moved to PSRAM.** Three remaining internal-heap holders on the CCA/cloud path: (1) `query_cca_config()` and `query_cca_device_info()` read into a PSRAM chunk buffer but accumulated into a default-allocator `std::string` with a **64 KB** cap — and `query_cca_config()` runs on the *main loop*, so a large node list spiked the internal floor at the worst possible moment; both accumulators are now `psram_string`, with `match_cca_to_uart()` taking a `const char *` and `set_cca_device_info()` gaining a `psram_string` overload so nothing round-trips through an internal temporary. (2) `get_cca_device_info()` returned a `std::string` copy of a member that was *already* a `psram_string`, putting a multi-KB copy on the internal heap from the httpd task — and the `/cca` page called it **twice** per request, the first time only to test `.empty()`. It now returns a `psram_string`, and a new `cca_device_info_empty()` probe replaces the throwaway copy. (3) The persistent cloud credentials (`cloud_token_` is a JWT of ~1–1.5 KB, plus refresh token, email, and expiry) were resident `std::string` members; they are now `psram_string`, with `cloud_http_json_()` taking the bearer as `const char *` and building its `Authorization` header in PSRAM.
- **Node/device/string struct contents moved to PSRAM.** `devices_`, `node_table_`, `strings_` and `inverters_` were already `psram_vector`/`psram_map`, but that only places the *container nodes* in PSRAM — every `std::string` member inside them still allocated its buffer from the internal heap as soon as it beat libstdc++'s 15-char SSO limit. `NodeTableData::long_address` is **exactly 16 chars**, so it beat it on every node, permanently; so did `DeviceData::barcode`, and so do the CCA labels in practice. The `device_addrs` / `mppt_labels` vectors were internal regardless of string length. At 40 nodes that was on the order of a hundred small, permanently-resident internal allocations — the fragmentation-prone kind. A new `node_string` / `node_vector` alias pair (PSRAM under ESP-IDF, plain `std::string`/`std::vector` otherwise) now backs every string member of those four structs, along with the maps keyed on a device address or string label. Unlike the other PSRAM moves this one reduces *permanent* internal residency rather than transient spikes. Cross-allocator `operator==`/`!=` overloads keep the many `node_string`-vs-`std::string` comparisons free of temporaries, and explicit `to_node_string()` / `to_std_string()` helpers mark every remaining allocator crossing (ESPHome sensor publishes, HTTP query strings, log lines) at the call site. The sensor-registration entry points now take `const char *` — ESPHome codegen passes string literals anyway.
- **BLE CCA caches moved to PSRAM.** The last internal-heap holders on the CCA-over-BLE path, and — like the struct members above — mostly *persistent* rather than transient: `cca_info_json_` (a DEVICE_INFO payload of ~1.7 KB), `cca_network_json_`, `cca_discovery_json_`, and the per-command `cca_net_results_` map stay resident for the life of the process and are restored from NVS at boot. `ble_response_buffer_` was worse than its final size suggests: a CCA reply reassembles from ~17-byte GATT notifications, so the vector regrows repeatedly on its way up. Also moved: the 128-char hex session key and the deferred/pending parameter strings that carry it. All the `ble_get_*_json_()` accessors now return `psram_string` instead of copying to the internal heap on every `/api/cca*` request, which makes `build_cca_info_json()` PSRAM end-to-end — both its sources now hand back a `psram_string`.
...
### Changed
- **ESPHome 2026.7.0 compatibility — BLE builds must stay on `version: recommended`.** ESPHome's `recommended` esp-idf toolchain is IDF 5.5.4 on both 2026.6.5 and 2026.7.0 and builds `cca_source: ble` cleanly; the everyday boards already use it, so they're unaffected. The break (issue #33) only hits an explicit `version: "6.0.x"` pin: that selects the pioarduino `prep_IDF6` platform — a *moving branch* that has advanced to deliver **IDF 6.0.2**, which relocated the Bluedroid headers (`esp_bt_defs.h` → `bt/host/bluedroid/api/include/api/`) off the global include path. ESPHome's `esp32_ble_tracker.h` still `#include`s it without requiring the `bt` component, so any BLE source fails with `fatal error: esp_bt_defs.h: No such file or directory`. Upstream ESPHome × IDF-6 issue, not ours; IDF-6 + BLE stays blocked until ESPHome adds `bt` to the BLE component's `REQUIRES`. The monitor-only IDF-6 build path (`boards/test-idf6-tigomonitor.yaml`) is unaffected because it pulls in no BLE. Documented in `docs/TROUBLESHOOTING.md`; the IDF-6 test board's header note updated.
