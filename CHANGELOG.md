...
### Changed
- **History resolution improves to 10 minutes.** The snapshot interval had been coarsened twice — 5 to 30 minutes, then 30 to 60 — not because anyone wanted less detail, but because each snapshot's flash writes were what triggered the crash below, so the only lever available was doing it less often. With the underlying fault addressed, cadence goes back to being a resolution decision, and 10 minutes is finer than the original 5-minute setting ever ran stably. The trade is retention, and it applies to both stores: each database holds a fixed number of readings however often you fill it, so six times the detail means a sixth of the span. Per-panel history now covers about 5 weeks rather than 7 months, and system-wide history about 7 months rather than 3.5 years. Lifetime energy totals are unaffected — each row stores energy since the previous snapshot, not a running total.
