...
### Fixed
- **Tigo cloud (`cloud_import`) TLS failed on ESP-IDF 6.0** — `Failed to verify certificate` / `mbedtls_ssl_handshake returned -0x3000` when logging in. ESP-IDF 6.0's certificate bundle dropped the `GlobalSign Root CA` root that Tigo's cross-signed `GTS Root R4` chain (mapi.tigoenergy.com, on Google Trust Services) anchors on — and `esp_crt_bundle` keys trust on the *presented* chain's top issuer, so the self-signed `GTS Root R4` still in the bundle couldn't help. The cloud client now verifies against a small pinned set of Google Trust Services roots (the ECDSA GTS Root R3/R4, embedded in `tigo_cloud_ca.h`) instead of the ESP-IDF bundle: fixes the missing-root case, unchanged on 5.x, and drops the ~64 KB full bundle from the build. R4 anchors Tigo's current chain. Additionally, `cloud_import` now force-enables `CONFIG_MBEDTLS_ECP_DP_SECP384R1_ENABLED`: GTS Root R4 has a P-384 key and Tigo's `WE1` intermediate is P-384-signed, so a build with secp384r1 disabled can neither parse the pinned root nor verify the chain — it fails CA parse with `-0x2100` (`X509_UNKNOWN_OID`). Verified — Tigo's live leaf validates against R3/R4 (openssl), the forced P-384 option overrides an explicit disable, and firmware builds green on IDF 5.x and 6.0.
