...
| High packet loss | Add `CONFIG_UART_ISR_IN_IRAM: "y"` ([UART Optimization](UART_OPTIMIZATION.md)) |
...
**Expected miss rate:** 0.02-0.04% is excellent for multi-drop RS485. For the deep dive, see [UART Optimization](UART_OPTIMIZATION.md).
...
See [UART Optimization](UART_OPTIMIZATION.md) for the full tuning guide.
...
2-8 KB is plenty on an ESP32-S3 at 38400 baud. This buffer comes from internal (DMA-capable) RAM, never PSRAM, so an oversized value wastes scarce internal heap — a 32 KB buffer silently spends a sixth of the usable internal RAM. See [UART Optimization](UART_OPTIMIZATION.md).
...
3. Confirm your config points esp_tsdb at the `RAR/esp_tsdb` fork's `tigomonitor` branch and uses the custom partition table.
...
**See also:** [Wiring](WIRING.md) · [UART Optimization](UART_OPTIMIZATION.md) · [Configuration](CONFIGURATION.md) · [← Back to README](../README.md)
