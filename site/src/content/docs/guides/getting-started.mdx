...
**Export your node table first** — web UI → Nodes → Export. Changing the flash
layout moves the region holding everything you've set on the device, and the
old contents don't move with it: panel names, inverter and string renames,
per-string ratings, the settings under Tools, and a saved CCA address. Nodes →
Import restores the lot afterwards, topology overrides included. Anything set
in YAML comes back on its own.
...
If the board is awkward to reach, the wireless route takes three steps — and
the middle one is easy to miss:

1. OTA your *current* layout with `allow_partition_access: true` added to the
   `ota:` block and nothing else changed. This is the permission; it grants
   nothing retroactively, so it has to be its own update.
2. `esphome upload --partition-table your-config.yaml` — **this is the step
   that changes the layout.** A plain `esphome run` never sends a partition
   table, no matter what `allow_partition_access` says. It will report success
   and leave your layout exactly as it was, which looks like it worked.
3. OTA the new firmware normally.
...
Either way stored history starts empty, and Home Assistant entities are
unaffected.
...
Then open the **History** tab. It fills one snapshot at a time, so it is empty
for the first half hour no matter what — but if it reports that the device clock
has not been set, that's the thing to fix before anything else. Daily energy, the
midnight reset and all stored history key off a real timestamp, and none of them
works until the device reaches a time server. The usual cause is a static IP
configured without a DNS server; see
[Static IP With No DNS](/esphome-tigomonitor/guides/troubleshooting/#static-ip-with-no-dns-breaks-the-clock-cca-sync-and-cloud-import).
