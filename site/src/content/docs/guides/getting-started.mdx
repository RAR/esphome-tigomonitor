...
**Then flash over USB.** An over-the-air update from 1.x won't work on its own,
because the 1.x image predates `allow_partition_access`. If the board is
awkward to reach, do it in two OTAs instead: first send your *current* layout
with `allow_partition_access: true` added and nothing else changed, then send
the new one. Either way stored history starts empty, and Home Assistant
entities are unaffected.

The same applies later if you enable `cca_source: ble` on an 8 MB board — that
needs a bigger app slot, so it is another layout change with the same
consequences. Export first.
