...
keeps a **permanent record on the device itself** — about seven months of system
history and five weeks per panel — so the History charts still work after a power
cut, a reboot, or a firmware update.
...
**A reading every 10 minutes.** Not every few seconds — flash memory wears out
if you write to it constantly. Your energy totals are unaffected by this; only
how finely the charts are drawn.

**About five weeks of per-panel detail.** Each panel keeps its most recent 5,404
readings and then starts overwriting the oldest. System-wide history keeps
longer — roughly seven months.
...
Phases 1–3 shipped. Per-snapshot system rollups + per-panel power are persisted at 10-min cadence; up to 48 panels supported across three lazy-opened panel DBs. Daily-rollup phase (Phase 4) and the volatile-history retirement (Phase 6) are tracked separately and not on the critical path.
...
### `system.tsdb` — system + per-inverter rollups (10-min cadence, 14 params)

| # | Name | Unit | Scale |
|---|------|------|-------|
| 0 | `total_p` | W | ×1 |
| 1 | `total_e` | kWh | ×100 (period delta — energy produced in the 10-min window) |
| 2–9 | `inv1_p` … `inv4_e` | W or kWh ×100 | per-inverter power and 10-min energy delta |
| 10 | `temp_avg` | °C | ×1 |
| 11 | `freq` | dHz | ×10 (currently 0 — wired but not extracted from telemetry) |
| 12 | `frames_lost` | count | ×1 |
| 13 | `wifi_rssi` | dBm | ×1 |

### `panels{0,1,2}.tsdb` — per-panel power (10-min cadence, 16 params each)
...
| DB | File size | Records | At 10-min cadence | Buffer pool |
|----|-----------|---------|-------------------|-------------|
| `system.tsdb` | 1 MB | ~32,700 records | ~227 days | 10 KB (PSRAM) |
| `panels{0,1,2}.tsdb` | 192 KB each | ~5,400 records | ~37.5 days each | 6 KB each (PSRAM) |
...
3. `xQueueSend` non-blocking — if the queue is full (4-deep), drop the sample with a log warning. Even at 10-min cadence the queue should never be more than 1 deep in steady state.
...
|----------|--------|------------|----------------|
| `/api/history/power?range=day` | `system.tsdb` | 10-min | ~144 |
| `/api/history/power?range=week` | `system.tsdb` | 10-min | ~1,008 |
| `/api/history/power?range=month` | `system.tsdb` | 10-min | ~4,320 |
| `/api/history/power?range=year` | `system.tsdb` | 10-min | ~32,700 — **capped by capacity, not by range**: the DB holds ~227 days, so `range=year` returns about 7.5 months, not 12 |
| `/api/history/panel?slot=N&range=…` | `panels{slot/16}.tsdb` | 10-min | one column read (~37.5 days of panel history available) |
...
- **Daily/monthly pre-aggregation** (`daily.tsdb` from the original plan) — month/year queries currently scan the system DB directly. Fine at this size, but 10-min cadence now means `range=year` is capacity-bound (~227 days); pre-aggregation or larger flash would be the fix if a true multi-year view is wanted.
