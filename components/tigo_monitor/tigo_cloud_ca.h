...
// instead of esp_crt_bundle: ESP-IDF 6.0's bundle dropped "GlobalSign Root CA",
// which Tigo's cross-signed GTS Root R4 chain relied on — and esp_crt_bundle
// anchors on the *presented* chain's top issuer, so the self-signed GTS Root R4
// still in the bundle didn't help. Providing the GTS roots lets mbedtls
// path-build directly, works on both IDF 5.x and 6.x, and drops the ~64 KB full
// cert bundle. Covers any current Google Trust Services intermediate (R1-R4);
// update these if Tigo ever moves off Google Trust Services.
//
